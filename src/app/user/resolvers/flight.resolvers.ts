...
@Injectable()
export class getFlights implements Resolve<any> {
  constructor(private data:DummyCarsHotelsVenuesFlightService, private Helpers:HelpersService) {}
    
  resolve(){
    
      return this.data.getFlights(1, 15).pipe(catchError((err)=>{
...
@Injectable()
export class GetFlightId implements Resolve<any> {
  constructor(private data:DummyCarsHotelsVenuesFlightService, private Helpers:HelpersService) {}
    
  resolve(route: ActivatedRouteSnapshot){
    
   return this.data.getFlightDetails(route.paramMap.get('id')).pipe(catchError((err)=>{
      setTimeout(() => this.Helpers.errorToast('', 'Error connecting to server, check internet connection'));
      return empty();
  }))
  }
