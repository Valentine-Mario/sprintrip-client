...
@Injectable()
export class GetVenues implements Resolve<any> {
  constructor(private data:DummyCarsHotelsVenuesFlightService, private Helpers:HelpersService) {}
    
  resolve(){
    
      return this.data.getVenues(1, 15).pipe(catchError((err)=>{
...
@Injectable()
export class GetVenueId implements Resolve<any> {
  constructor(private data:DummyCarsHotelsVenuesFlightService, private Helpers:HelpersService) {}
    
  resolve(route: ActivatedRouteSnapshot){
    
   return this.data.getVenueDetails(route.paramMap.get('id')).pipe(catchError((err)=>{
    setTimeout(() => this.Helpers.errorToast('', 'Error connecting to server, check internet connection'));
    return empty();
  }))
  }
