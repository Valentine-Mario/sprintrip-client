...
hide_group:Boolean=true;
  ...
  addBooking(){
    ...
    if(localStorage.getItem('invite-token')){
      this.bookingService.addVenueBookingInvitedUser(this.venue.body.message._id, formData).subscribe(response=>{
        this.spin=false;
      ...
      this.bookingService.addVenueBookingUser(this.venue.body.message._id, formData).subscribe(response=>{
        this.spin=false;
        if(response.status==200){
          this.Helpers.successToast('', 'booking made successfully');
          this.modalService.dismissAll();
      ...
      this.bookingService.addVenueBookingNonUser(this.venue.body.message._id, formData).subscribe(response=>{
        this.spin=false;
        if(response.status==200){
          this.Helpers.successToast('', 'booking made successfully');
          this.modalService.dismissAll();
